...
class UserTest < ActiveSupport::TestCase
  ...
  test "should follow and unfollow a user" do
    ...
    assert archer.followers.include?(michael) # archerのフォロワーにmichaelいるよね？
  ...
  test "feed should have the right posts" do
    ...
    lana.microposts.each do |post_following|
      assert michael.feed.include?(post_following) # michaelのフィードにlanaのポストが含まれているか
    end
    # フォロワーがいるユーザー自身の投稿を確認
    michael.microposts.each do |post_self|
      assert michael.feed.include?(post_self) # michaelのフィードにmichael自身のポストが含まれているか
      assert_equal michael.feed.distinct, michael.feed # distinctは重複抜きで要素を返す
    end
    # フォローしていないユーザーの投稿を確認
    archer.microposts.each do |post_unfollowed|
      assert_not michael.feed.include?(post_unfollowed) # michaelのフィードにフォローしていないarcherのポストが含まれていない
    end
  end
end
